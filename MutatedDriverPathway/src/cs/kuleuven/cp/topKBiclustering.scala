...
object topKBiclustering extends App {
	  ...
	  cp.maximize(biclusterArea)  subjectTo {
	    ...
	    for(c <- Cols){
	      ...
	      cp.post(varCols(c) ==> aux)
		   ...
		   val selectedCols = (0 until varCols.size).filter(c => varCols(c).getValue == 1)
		   val selectedRows = (0 until varRows.size).filter(r => varRows(r).getValue == 1)
		   solution.setObjValue(selectedCols.size * selectedRows.size)	   
